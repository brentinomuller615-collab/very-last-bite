  ...
  return (
    <div className={`p-6 rounded-2xl border transition-all ${
      ...
      {(order.status === "Reserved" && !success) && (
        <p className="text-sm text-[#4B5563] mb-5 flex items-center">
          ...
          <div className="flex gap-2">
            {isVerifying && (
              <Button 
                ...
                className="!py-3 !rounded-xl !px-5 w-auto"
              >
                Cancel
              </Button>
            )}
            <Button onClick={handleVerify} className="!py-3 !rounded-xl flex-1">
              Verify Pickup
            </Button>
          </div>
        </div>
